# Controlador: InventarioController (Admin)

**Ubicación:** `app/Http/Controllers/Admin/InventarioController.php`

## Descripción
Este controlador permite a los administradores del restaurante realizar la gestión del stock de insumos y materias primas ligadas a los productos del menú. También audita los movimientos de inventario.

## Métodos
* **`index()`**: Muestra la lista de productos e insumos con su stock actual, alertas de stock mínimo y la bitácora histórica de movimientos de existencias.
* **`storeProduct(Request)`**: Crea y registra un nuevo artículo de inventario y le asigna una cantidad de existencias iniciales.
* **`registrarMovimiento(Request, $id)`**: Registra un movimiento manual de stock (Entrada o Salida) de un producto, actualizando las existencias disponibles e insertando el motivo en la bitácora.
* **`destroyProduct($id)`**: Elimina un artículo del catálogo de inventario de forma física.
