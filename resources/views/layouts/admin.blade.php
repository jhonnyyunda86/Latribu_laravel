...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'La Tribu - Admin') }}</title>
    ...
    <body class="font-sans antialiased bg-[#fdfbf7] text-[#2c1d11]" x-data="{ mobileSidebarOpen: false }">
        <div class="min-h-screen flex">
            ...
            <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
                <!-- Top Navbar (Charcoal & Gold Header matching welcome theme) -->
                <header class="bg-[#121619] border-b border-[#d4af37]/20 h-16 flex items-center justify-between px-6 z-30 shadow-md">
                    <div class="flex items-center gap-4">
                        ...
                        <div class="text-sm font-bold uppercase tracking-wider text-[#d4af37]">
                            Dashboard Administrador
                        </div>
                    </div>

                    <!-- Right Profile Status & Logout -->
                    <div class="flex items-center gap-4">
                        <div class="flex flex-col text-right leading-tight hidden sm:flex text-gray-300">
                            <span class="text-[10px] font-bold text-[#d4af37] uppercase tracking-widest">Admin</span>
