<x-guest-layout>
    ...
    <div class="p-8 sm:p-12 flex flex-col justify-between bg-tribu-darkBg text-white relative min-h-[450px] md:min-h-[500px]">
        ...
        <div class="my-auto space-y-6 text-center flex flex-col items-center">
            ...
            <div class="space-y-3">
                <p class="text-xs text-gray-300 font-light leading-relaxed max-w-xs mx-auto italic font-serif">
                    Accede al sistema de gestión para administrar tus pedidos, inventarios y procesos de forma rápida y elegante.
                </p>
            </div>
            ...
            <div class="pt-2 w-full max-w-[200px]">
                @if (Route::has('register'))
                    <a href="{{ route('register') }}" class="block w-full py-3 bg-tribu-gold hover:bg-yellow-600 text-tribu-darkBg font-bold rounded-full text-[10px] tracking-widest uppercase transition text-center shadow-md">
                        Crear cuenta
                    </a>
                @endif
            </div>
        ...
        <div class="text-center text-[10px] text-gray-500 uppercase tracking-wider">
            ¿No tienes una cuenta? <a href="{{ route('register') }}" class="text-tribu-gold hover:underline font-bold">Regístrate</a>
        </div>
    </div>

    <!-- Lado Derecho (Formulario Limpio con Acentos Dorados / Fondo Crema Claro) -->
    <div class="p-8 sm:p-12 bg-[#fdfbf7] text-tribu-dark flex flex-col justify-between min-h-[450px] md:min-h-[500px]">
        ...
        <!-- Formulario e Inputs -->
        <div class="my-auto space-y-6 pt-4">
            <div class="text-center space-y-1">
                <h3 class="text-2xl font-serif tracking-wide text-tribu-dark italic">Iniciar Sesión</h3>
                <p class="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Ingresa tus datos para continuar</p>
            </div>
                ...
                <div class="flex items-center justify-between text-[10px] font-bold text-gray-500 uppercase tracking-wider">
                    <label for="remember_me" class="inline-flex items-center cursor-pointer">
                        <input id="remember_me" 
                               type="checkbox" 
                               name="remember" 
                               class="rounded border-gray-300 text-tribu-gold focus:ring-tribu-gold h-3.5 w-3.5 bg-white">
                        <span class="ms-1.5">Recordarme</span>
                    </label>
                    @if (Route::has('password.request'))
                        <a class="hover:underline text-tribu-gold" href="{{ route('password.request') }}">
        ...
        @if (Route::has('register'))
            <div class="text-center text-[10px] text-gray-500 uppercase tracking-wider">
                ¿No tienes cuenta? <a href="{{ route('register') }}" class="text-tribu-gold hover:underline font-bold">Regístrate</a>
            </div>
        @endif
