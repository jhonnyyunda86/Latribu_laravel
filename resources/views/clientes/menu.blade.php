@php
    // Las categorías y productos son provistos por el controlador
@endphp

<x-cliente-layout>
    <x-slot name="header">
        {{ __('Carta Digital & Pedido a Domicilio') }}
    </x-slot>

    <!-- Envoltorio Alpine.js para gestionar búsqueda, filtros y carrito de compras -->
    <div x-data="{ 
            searchQuery: '', 
            selectedCategory: '',
            cart: [],
        ...
        <div class="bg-[#121619] rounded-2xl p-6 text-white shadow-xl relative overflow-hidden border border-[#d4af37]/20">
            ...
            <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <span class="text-[9px] tracking-[0.3em] text-[#d4af37] font-bold uppercase block mb-1.5">• DIRECTO A TU HOGAR •</span>
                    <h3 class="text-2xl font-serif tracking-wide italic mb-1">Pide tu comida favorita</h3>
                    <p class="text-xs text-gray-400 max-w-xl font-light">Explora nuestra variada carta de platos familiares, arma tu pedido y recíbelo en la comodidad de tu casa con factura y confirmación instantánea.</p>
                </div>
            </div>
        </div>

        <!-- Grid de dos columnas: Carta y Pedido -->
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            
            <!-- CARTA DE PRODUCTOS (2/3 de ancho) -->
                ...
                <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-200 space-y-4">
                    <div class="relative">
                        ...
                        <input x-model="searchQuery" 
                               type="text" 
                               placeholder="¿Qué se te antoja hoy? Busca por nombre o ingredientes..." 
                            ...
                            <div class="px-5 pb-5 pt-3 border-t border-gray-100 flex flex-col gap-3 bg-gray-50/50 rounded-b-2xl">
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="text-right">
                                        ...
                                        <span class="text-xs font-bold font-mono text-gray-700" x-text="product.stock + ' uds'"></span>
                                    </div>
                                </div>
                                <button @click="addToCart(product)"
                                        :disabled="!product.disponible || (product.stock > 0 && (cart.find(i => i.id === product.id)?.qty >= product.stock))"
            ...
            <!-- DETALLE DEL PEDIDO Y ENVÍO (Columna derecha) -->
                ...
                <div class="bg-white rounded-2xl p-6 shadow-md border border-gray-200 space-y-4">
                    ...
                    <template x-if="cart.length === 0">
                        <div class="py-8 text-center text-gray-400 space-y-2">
                            ...
                            <p class="text-xs font-light">Tu carrito está vacío. Agrega tus platillos desde la carta digital.</p>
                        </div>
                    </template>
                    ...
                    <template x-if="cart.length > 0">
                        <div class="space-y-4">
                            <!-- Items en carrito -->
                            <div class="max-h-56 overflow-y-auto divide-y divide-gray-100 pr-1">
                                <template x-for="item in cart" :key="item.id">
                                    <div class="py-2.5 flex items-center justify-between gap-2">
                                        ...
                                        <div class="flex items-center gap-2 bg-gray-100 rounded-lg p-1 shrink-0">
                                            ...
                                            <span class="text-xs font-mono font-bold w-5 text-center" x-text="item.qty"></span>
                                            <button @click="updateQty(item.id, 1)" class="w-5 h-5 bg-white text-[#2c1d11] rounded shadow-sm hover:bg-gray-200 transition font-bold flex items-center justify-center text-xs">+</button>
                                        </div>
                            ...
                            <div class="border-t border-gray-100 pt-3 flex justify-between items-center bg-gray-50 p-3 rounded-xl">
                                <span class="text-xs uppercase tracking-wider font-bold text-gray-500">Total Pedido</span>
                                <span class="text-md font-bold font-mono text-[#2c1d11]" x-text="getCartTotal()"></span>
                            </div>

                            <!-- Formulario de envío a domicilio -->
                            <form action="{{ route('cliente.pedidos.store') }}" method="POST" class="space-y-4">
                                @csrf
                                
                                <!-- Dirección de entrega -->
                                <div>
                                    <label class="text-[10px] font-bold text-gray-500 block uppercase tracking-widest mb-1">Dirección de Entrega *</label>
                                    <input type="text" name="direccion" x-model="direccion" required placeholder="Ej: Calle 45 # 12-34, Apto 302" class="w-full py-2.5 px-3 bg-gray-50 border border-gray-200 rounded-xl text-xs font-medium focus:ring-[#d4af37] focus:border-[#d4af37]">
                                </div>

                                <!-- Teléfono -->
                                <div>
                                    <label class="text-[10px] font-bold text-gray-500 block uppercase tracking-widest mb-1">Teléfono de Contacto *</label>
                                    <input type="text" name="telefono" x-model="telefono" required placeholder="Ej: 3001234567" class="w-full py-2.5 px-3 bg-gray-50 border border-gray-200 rounded-xl text-xs font-medium focus:ring-[#d4af37] focus:border-[#d4af37]">
                                </div>

                                <!-- Dynamic hidden inputs del carrito -->
                                ...
                                <!-- Observaciones -->
                                <div>
                                    <label class="text-[10px] font-bold text-gray-500 block uppercase tracking-widest mb-1">Indicaciones Adicionales</label>
                                    <textarea name="observaciones" x-model="observaciones" placeholder="Ej: Portería, reja blanca, timbre dañado..." class="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-xs h-16 focus:ring-[#d4af37] focus:border-[#d4af37] font-light leading-relaxed"></textarea>
                                </div>

                                <button type="submit" 
                                        :disabled="cart.length === 0 || !direccion || !telefono"
                                        class="w-full py-3 bg-[#d4af37] disabled:bg-gray-250 disabled:text-gray-400 hover:bg-yellow-600 text-[#121619] font-bold rounded-xl text-xs uppercase tracking-wider transition shadow-sm flex items-center justify-center gap-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" />
                                    </svg>
                                    Realizar Pedido a Domicilio
                                </button>
                            </form>
                        </div>
                    </template>
        ...
        <div class="fixed bottom-6 right-6 lg:hidden z-40" x-show="cart.length > 0" x-transition>
            <button @click="mobileCartOpen = true" class="bg-[#d4af37] text-[#121619] p-4 rounded-full shadow-2xl flex items-center gap-2 font-bold animate-pulse">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                </svg>
                <span class="text-xs uppercase tracking-wider font-extrabold" x-text="'Pedido (' + getCartItemCount() + ')'"></span>
            </button>
        </div>

        <!-- DETALLE DE CARRITO MÓVIL (MODAL BOTTOM-SHEET) -->
